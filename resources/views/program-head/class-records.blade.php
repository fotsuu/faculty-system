...
@section('title', 'Class Records - Program Head Dashboard')
    ...
    <div class="section">
        <div class="section-header">
            <div>
                ...
                <div style="font-size: 13px; color: #64748b;">All class record uploads from faculty in your department</div>
            </div>
                ...
                @foreach($classRecordGroups as $group)
                    <div class="class-record-card" style="background: white; border-radius: 16px; border: 1px solid #edf2f7; box-shadow: 0 15px 35px rgba(15, 23, 42, 0.06); display: flex; flex-direction: column; overflow: hidden;">
                        <div style="padding: 22px; border-bottom: 1px solid #edf2f7; background: #f8fafc;">
                            ...
                            <div style="font-size: 12px; color: #64748b;">Uploaded by {{ $group->uploaded_by }}</div>
                            <div style="font-size: 12px; color: #64748b; margin-top: 8px;">{{ optional($group->uploaded_at)->format('M d, Y g:i A') }}</div>
                        </div>
                        ...
                        <div style="padding: 16px 22px; border-top: 1px solid #edf2f7; background: #f8fafc;">
                            <a href="{{ route('program-head.class-records.view', ['user_id' => $group->user_id ?? $group->id, 'file_name' => $group->file_name]) }}" 
        ...
        @else
            <div style="text-align: center; padding: 60px 20px; background: white; border-radius: 12px; border: 1px solid #edf2f7;">
                ...
                <p style="font-size: 14px; color: #64748b;">No faculty in your department have uploaded class record files yet.</p>
            </div>
        @endif
    </div>
