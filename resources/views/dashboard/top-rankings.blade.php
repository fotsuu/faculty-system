@php
    $layout = $layout ?? 'layouts.faculty_new';
    $title = $title ?? 'Top Rankings - DSSC CRMS';
    $pageTitle = $pageTitle ?? 'Top Performing Students';
...
@section('content')
    <div class="section">
        <div class="section-header" style="margin-bottom: 18px; display:flex; justify-content:space-between; align-items:flex-end; gap:12px;">
            <div>
                ...
                <div style="font-size: 13px; color: #64748b;">
                    ...
                    @if(!empty($selectedSemester)) — Semester: {{ $selectedSemester }} @endif
                </div>
            </div>
            <div style="display:flex; gap: 10px; flex-wrap: wrap; align-items:center;">
                <a href="{{ route($backRoute) }}" style="font-size:12px; color:#1e3c72; text-decoration:none; font-weight:700;">← Back to Dashboard</a>
            </div>
        </div>
        ...
        @else
            <div style="padding: 40px; text-align: center; color:#64748b; background:white; border:1px solid #edf2f7; border-radius:12px;">
                ...
                <div style="font-size:13px; color:#94a3b8;">Upload class records and apply filters to see top performers.</div>
            </div>
        @endif
    </div>
@endsection